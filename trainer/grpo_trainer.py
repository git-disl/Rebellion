    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # accruacy reward
                import logging 
                # logging.info("before: {}".format(output_reward_func))
                if i==0:
                    for j  in range(len(output_reward_func)):
                        output_reward_func[j] = (1-self.mixture_ratio) * output_reward_func[j]
                    
                # safety reward
                elif i==2:
                    for j in range(len(output_reward_func)):
                        output_reward_func[j] = self.mixture_ratio * output_reward_func[j]
                # logging.info("after: {}".format(output_reward_func))

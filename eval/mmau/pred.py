...
class TestArguments:
    ...
    model_dir: Optional[str] = field(default=None, metadata={"help": "model dir"})
    data_dir: Optional[str] = field(default=None, metadata={"help": "test file"})
    audio_dir: Optional[str] = field(default=None, metadata={"help": "audio dir"})
    output_dir: Optional[str] = field(default=None, metadata={"help": "output file for test"})
    batch_size: Optional[int] =  field(default=16, metadata={"help": "batch size"})
    reward_format: Optional[str] =  field(default="cot", metadata={"help": "format"})
...
def _get_message(obj_dict, args):
    ...
    # If you want to improve the thinking process, uncomment the next line and design your strategy.
        question_template = f"{obj_dict['question']} {choice_str} Please think about this question as if you were a human pondering deeply. It's encouraged to include self-reflection or verification in the reasoning process. Output the thinking process in <think> </think> and final answer in <answer> </answer>."
...
def main():
    ...
    out_dir = os.path.abspath(os.path.dirname(data_args.output_dir))
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    audio_processor = AutoProcessor.from_pretrained(data_args.model_dir)
    audio_model = Qwen2AudioForConditionalGeneration.from_pretrained(data_args.model_dir, device_map="auto")
    ...
    for i in tqdm(range(0, len(datas), batch_size)):
        ...
        for bd in batch_data:
            batch_messages.append(_get_message(bd,data_args))
    ...
    for input_example, model_output in zip(datas, all_outputs):
        ...
        result["full_output"] = model_output
        model_answer = extract_answer(original_output).strip()
        # Create a result dictionary for this example
        result["output"] = model_answer
        final_output.append(result)

    # Save results to a JSON file
    output_path = data_args.output_dir
